...
public static class FhirResourceMapper
{
    ...
    #region Dashboard Mappings

    public static AlertDto ToAlertDto(Flag flag, string? patientName = null)
    {
        return new AlertDto(
            Id: flag.Id,
            Title: flag.Code.Text ?? flag.Code.Coding.FirstOrDefault()?.Display ?? "Alert",
            Description: flag.Code.Text,
            Priority: MapFlagCategoryToPriority(flag.Category.FirstOrDefault()),
            Status: flag.Status?.ToString().ToLowerInvariant() ?? "active",
            PatientId: flag.Subject?.Reference?.Replace("Patient/", ""),
            PatientName: patientName,
            Timestamp: flag.Period?.Start ?? DateTime.UtcNow.ToString("o")
        );
    }

    public static ActivityDto ToActivityDto(AuditEvent auditEvent)
    {
        return new ActivityDto(
            Id: auditEvent.Id,
            Type: auditEvent.Type?.Display ?? auditEvent.Action?.ToString() ?? "unknown",
            ResourceType: auditEvent.Entity.FirstOrDefault()?.What?.Type ?? "Resource",
            Description: auditEvent.Entity.FirstOrDefault()?.What?.Display ?? "",
            User: auditEvent.Agent.FirstOrDefault()?.Who?.Display ?? "System",
            Timestamp: auditEvent.Recorded?.ToString("o") ?? DateTime.UtcNow.ToString("o")
        );
    }
    ...
    private static string MapFlagCategoryToPriority(CodeableConcept? category)
    {
        var code = category?.Coding.FirstOrDefault()?.Code;
        return code switch
        {
            "admin" => "low",
            "clinical" => "high",
